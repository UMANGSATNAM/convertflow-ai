...
function ThemeEditorApp() {
    const { device, setDevice, saveSettings, selectedBlockId, activeBlock, themeName, blocks, undo, redo, canUndo, canRedo, hasUnsavedChanges, fetcher } = useThemeEditor();
            ...
            <header style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 200, justifyContent: 'flex-end' }}>
                    <button
                        ...
                        onClick={undo}
                        disabled={!canUndo}
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, border: '1px solid #ebebeb', background: '#fff', cursor: canUndo ? 'pointer' : 'not-allowed', borderRadius: 6, color: canUndo ? '#303030' : '#c9cccf', opacity: canUndo ? 1 : 0.5 }}
                    >
                    ...
                    <button
                        ...
                        onClick={redo}
                        disabled={!canRedo}
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, border: '1px solid #ebebeb', background: '#fff', cursor: canRedo ? 'pointer' : 'not-allowed', borderRadius: 6, color: canRedo ? '#303030' : '#c9cccf', opacity: canRedo ? 1 : 0.5 }}
                    >
                    ...
                    {isSaving && <span style={{ fontSize: 12, color: '#6d7175' }}>Saving...</span>}
                    {hasUnsavedChanges && !isSaving && <span style={{ fontSize: 11, color: '#916a00', background: '#fff4d1', padding: '2px 6px', borderRadius: 4, fontWeight: 500 }}>Unsaved</span>}
                    <button
                        onClick={saveSettings}
                        disabled={!selectedBlockId || isSaving}
                        style={{
                            height: 32, padding: '0 14px',
                            background: hasUnsavedChanges ? '#008060' : (selectedBlockId ? '#303030' : '#c9cccf'),
